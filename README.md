...
Le frontend démarre par défaut sur `http://localhost:3000` et appelle le backend via `REACT_APP_BACKEND_URL` (par défaut `http://localhost:8000`).
...
Depuis `backend/` :
```bash
# utilise BACKEND_URL si défini, sinon REACT_APP_BACKEND_URL, sinon http://localhost:8000
